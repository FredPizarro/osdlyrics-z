...
La caché local se guarda en C:\Lyrics
"""
...
ASSOCIATIONS_FILE = CACHE_DIR / 'associations.json'

HEADERS = {
    'User-Agent': 'OSDLyricsAIMP/1.0 (Windows; compatible)',
...
def _cache_path(title: str, artist: str) -> Path:
    # Formato: Artista - Título.lrc
    # Si falta artista, solo Título.lrc
    if artist:
        name = f"{artist} - {title}"
    else:
        name = title
...
# _cache_key ya no es necesaria pero la dejamos por compatibilidad si algo la llama, 
# aunque _cache_path ya no la usa.
def _cache_key(title: str, artist: str) -> str:
    raw = f"{title.lower().strip()}||{artist.lower().strip()}"
    return hashlib.md5(raw.encode('utf-8')).hexdigest()

def _load_associations() -> dict:
    """Carga el archivo de asociaciones."""
...
def _save_associations(assoc: dict):
    """Guarda el archivo de asociaciones."""
    try:
...
def save_association(title: str, artist: str, lrc_path: str):
    """Asocia una pista del reproductor con una ruta de archivo LRC."""
    key = f"{title}||{artist}"
...
def _load_from_association(title: str, artist: str):
    ...
    if lrc_path:
        ...
        if p.exists():
            ...
            # Archivo ya no existe, limpiar asociación
...
# ... (save_cache_file usa _cache_path, así que automáticamente usará el nuevo nombre) ...
    ...
    try:
        for file in CACHE_DIR.glob("*.lrc"):
            ...
            if query_norm in file.stem.lower():
                # Intentar parsear "Artista - Título" del nombre
                ...
                continue # Ya encontrado por nombre, saltar lectura contenido
            
            # 2. Buscar en contenido (fallback para archivos hash o sin match en nombre)
            try:
...
def fetch_from_netease(title: str, artist: str) -> str | None:
    ...
    # Intentar match exacto primero, luego el primero
    best = None
    for song in songs:
...
def search_local_lyrics(query: str) -> list[dict]:
    """Busca archivos LRC locales que coincidan con la query."""
    results = []
    query_norm = query.lower().strip()
    try:
        if CACHE_DIR.exists():
            for file in CACHE_DIR.glob("*.lrc"):
                # El nombre de archivo es hash, pero podemos intentar leer contenido
                # o simplemente listar si los archivos se llamaran legiblemente.
                # Como usamos hash en _cache_key, no podemos buscar por nombre de archivo legible.
                # A MENOS que cambiemos la estrategia de guardado o leamos contenido.
                # Para simplificar y ser útil al usuario, vamos a buscar en el contenido
                # las etiquetas [ti:] y [ar:] o asumir que el usuario renombró archivos manualmente.
                
                # INTENTO MEJORADO: Buscar en contenido (lento pero efectivo)
                try:
                    content = file.read_text(encoding='utf-8', errors='ignore')
                    if query_norm in content.lower():
                        ...
                        results.append({
                            ...
                            'artist': ar,
                            'title': ti,
                            'id': str(file), # Ruta completa como ID
                            'album': ''
                        })
                except:
                    pass
        ...
        if status == 200:
            ...
            for item in data:
                ...
                results.append({
                    ...
                    'rating': '-',      # API no provee rating
                    'downloads': '-'    # API no provee downloads
                })
    except Exception as e:
        logger.error("Error buscando LRCLIB: %s", e)

    # 2. NetEase (music.163.com)
    try:
        netease_songs = _netease_search(query, "") # Pasamos query como title, artist vacio
        for song in netease_songs:
            # NetEase search retorna canciones, no lyrics directos.
            # Asumimos que tienen lyrics (se verifica al descargar).
            ...
            results.append({
                ...
                'rating': '★' * (song.get('popularity', 0) // 20), # 0-100 -> 0-5 stars
                'downloads': '-'
            })
    except Exception as e:
...
if __name__ == '__main__':
    ...
    # Test
    # result = get_lyrics("Bohemian Rhapsody", "Queen", use_cache=False)
    # if result: print(result[:100])
    
    # Test Busqueda
    print("Buscando 'Queen'...")
    cand = search_online_lyrics("Queen")
    for c in cand[:3]:
        print(c)
